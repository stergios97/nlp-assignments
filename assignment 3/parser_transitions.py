...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.stack = ["ROOT"]                 # The stack with "ROOT" as the top element
        self.buffer = list(sentence)          # The buffer as a list
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":                              # Shifting a word from the buffer to the stack
            word = self.buffer.pop(0)                       # Removing the first word from the buffer
            self.stack.append(word)                         # Appending it to the stack
        elif transition == "LA":                           # Left-arc operation
            dependent = self.stack.pop(-2)                  # Removing the second-to-last word from the stack as the dependent
            head = self.stack[-1]                           # Removing the last word on the stack as the head
            self.dependencies.append((head, dependent))     # Adding the resulting dependency to the dependency list
        elif transition == "RA":                           # Right-arc operation
            dependent = self.stack.pop()                    # Removing the top word from the stack as the dependent 
            head = self.stack[-1]                           # Removing the last word on the stack as the head
            self.dependencies.append((head, dependent))     # Adding the resulting dependency to the dependency list
        else:
...
class DummyModel(object):
    ...
    def unidirectional_predict(self, partial_parses):
        ...
        """
        return [("RA" if pp.stack[1] == "right" else "LA") if len(pp.buffer) == 0 else "S" for pp in partial_parses]
