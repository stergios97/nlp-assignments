...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # 1)
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.embed_size * n_features, hidden_size)))
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.Tensor(hidden_size)))

        # 2)
        self.dropout = nn.Dropout(p=dropout_prob)
        
        # 3)
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(n_classes, hidden_size)))
        ...
        ###       for training networks than random uniform initialization (exploding and vanishing gradients).
    ...
    def forward(self, w):
        ...
        x = torch.matmul(x, self.embed_to_hidden_weight.t()) + self.embed_to_hidden_bias
        x = nn.ReLU()(x)
        x = self.dropout(x)
        logits = torch.matmul(x, self.hidden_to_logits_weight.t()) + self.hidden_to_logits_bias
